...
void console() {
    ...
    const char* msg_for_help = "----------------------------------------\n"
        ...
        "2 - получить перевод английского слова\n"
        "3 - напечатать дерево\n"
        "4 - упростить дерево\n"
        "5 - выйти\n"
        "----------------------------------------";
    const char* msg_for_eng = "Введите слово на английском: ";
    const char* msg_for_rus = "Введите слово по-русски: ";
    const char* msg_for_count = "Введите кол-во обращений к слову: ";
    const char* msg_for_translate = "Ваше слово по-русски: ";
    const char* msg_for_print = "Ваше дерево: ";
    const char* msg_for_sort = "Дерево упрощено!";
    ...
    while (true) {
        ...
        if (ch == '1') {
            ...
            cout << msg_for_rus << endl;
            cin.getline(rus, SIZE_OF_WORD);;

            cout << msg_for_count << endl;
            cin >> c;

            t.add(new node(eng, rus, c));
        }
        else if (ch == '2') {
            cout << msg_for_eng << endl;
            cin.get();
